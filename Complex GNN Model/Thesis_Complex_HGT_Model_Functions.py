...
import Thesis_Complex_HGT_Model_Functions as TCHGTMFs
...
def Read_Tracking_Data_4_Augmented_Training_Shots_Frames_On_1_Same_Target_Goal_4_All_Models(  ):
    ...
    Total_Goals_Scored_Throughout_Dataset = TCHGTMFs.Count_Total_Goals_Scored( df = augmented_training_shots_on_1_same_target_goal_df )
...
def Read_Tracking_Data_4_Test_Shots_Frames_On_1_Same_Target_Goal_4_All_Models(  ):
    ...
    Total_Goals_Scored_Throughout_Dataset = TCHGTMFs.Count_Total_Goals_Scored( df = test_shots_on_1_same_target_goal_df )
...
def Browse_and_Select_Reading_Preprocessed_Tracking_Data_4_PyG_GNN_Models( training_or_test_data ):
    ...
    Total_Goals_Scored_Throughout_Dataset = TCHGTMFs.Count_Total_Goals_Scored( df = preprocessed_shots_on_1_same_target_goal_df )
...
#######################################################################
#         COMPLEX PyG GNN MODEL-RELATED CLASSES & FUNCTIONS           #
#######################################################################
...
def Create_Soccer_Graph_Adjacency_Matrix_Graph_Edges_and_Edge_Features( df_row, want_directed_or_undirected_edges = "undirected" ):
    ...
    if want_directed_or_undirected_edges == "directed":   # Create Uni-Directional Edges
        ...
        Directed_Ball_Player_Edge_Features_tensor = torch.tensor( np.array( [ TCHGTMFs.Get_Flattened_Node_or_Edge_Features( features = df_row[f"Dist_Between_Ball_{Player_ID}"] ) for Player_ID in range(1, Num_Players + 1) ] ), dtype = torch.float )   # Tensor Shape/Dim == [#Edges, #Edge-Features]
...
def Create_Soccer_Heterogeneous_Graph_Data_From_Tracking_DataFrame_Rows( df_row, want_directed_or_undirected_edges = "undirected", simple_or_complex_graphs = "simple" ):
    ...
    Ball_Node_Features = np.array( [ TCHGTMFs.Get_Flattened_Node_or_Edge_Features( features = df_row[ Ball_Features_Columns_dict[simple_or_complex_graphs] ].values ) ] )
    ...
    Ball_Node_Position_Coordinates = np.array( [ TCHGTMFs.Get_Flattened_Node_or_Edge_Features( features = df_row[ Ball_Node_Position_Coordinates ].values ) ] )
    ...
    Players_Node_Features = np.array( [ TCHGTMFs.Get_Flattened_Node_or_Edge_Features( features = df_row[ Players_Features_Columns_dict[simple_or_complex_graphs](Player_ID) ].values ) for Player_ID in range(1, Num_Players + 1) ] )
    ...
    Players_Node_Position_Coordinates = np.array( [ TCHGTMFs.Get_Flattened_Node_or_Edge_Features( features = df_row[ Players_Node_Position_Coordinates(Player_ID) ].values ) for Player_ID in range(1, Num_Players + 1) ] )
    ...
    if want_directed_or_undirected_edges == "directed":
        ...
        Directed_Ball_Player_Edge_Indices_tensor, Directed_Ball_Player_Edge_Features_tensor, Directed_Player_Player_Edge_Indices_tensor, Directed_Player_Player_Edge_Features_tensor = TCHGTMFs.Create_Soccer_Graph_Adjacency_Matrix_Graph_Edges_and_Edge_Features( df_row = df_row, want_directed_or_undirected_edges = want_directed_or_undirected_edges )
    ...
    if want_directed_or_undirected_edges == "undirected":
        ...
        Undirected_Ball_Player_Edge_Indices_tensor, Undirected_Ball_Player_Edge_Features_tensor, Undirected_Player_Player_Edge_Indices_tensor, Undirected_Player_Player_Edge_Features_tensor = TCHGTMFs.Create_Soccer_Graph_Adjacency_Matrix_Graph_Edges_and_Edge_Features( df_row = df_row, want_directed_or_undirected_edges = want_directed_or_undirected_edges )
...
class Soccer_Heterogeneous_Graphs_Dataset( InMemoryDataset ):
    ...
    Input: pre_transform (torch_geometric.transforms): A Transformation Function/Method From the `torch_geometric.transforms`, That Takes In a `torch_geometric.data.Data` Object or `torch_geometric.data.HeteroData` Object & Returns a Transformed Version of It
...
def Plot_CV_Splitted_Indices( PyG_dataset, cv_strategy, figure_axes, num_K_splits = 5, lw = 10 ):
    """
    Function taken from --> https://scikit-learn.org/stable/auto_examples/model_selection/plot_cv_indices.html#sphx-glr-auto-examples-model-selection-plot-cv-indices-py
