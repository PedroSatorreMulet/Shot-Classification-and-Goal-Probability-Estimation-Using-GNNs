...
def Plot_Calibration_Curve_of_Unseen_Test_Set( y_test_true_class_labels, y_test_pred_class_probabilities, positive_label = 1.0, num_bins = 100, bin_width_strategy = "uniform", figure_size = (8, 6) ):
    """
    Plots the model's calibration curve for the unseen test set

    Input: y_test_true_class_labels (array-like) = True class labels of the unseen test set
    Input: y_test_pred_class_probabilities (array-like) = Predicted class labels' probabilities of the unseen test set
    Input: positive_label (float) = The label of the positive class;  Default == 1.0
    Input: num_bins (int) = Number of bins to use for calibration;  Default == 100
    Input: bin_width_strategy (str) = Strategy for the width of bins;  Options == {"uniform", "quantile"};  Default == "uniform"
    Input: figure_size (tuple) = Size of the figure;  Default == (8, 6)
    
    """
    
    True_Probabilities, Predicted_Probabilities = calibration_curve( y_test_true_class_labels, y_test_pred_class_probabilities, pos_label = positive_label, n_bins = num_bins, strategy = bin_width_strategy )

    plt.figure( figsize = figure_size )


    plt.plot( [0, 1], [0, 1], color = "navy", linestyle = '--', label = "Perfectly Calibrated Model" )   # Perfectly Calibrated Model Curve

    plt.plot( Predicted_Probabilities, True_Probabilities, color = "darkorange", marker = "o", label = "Model's Actual Calibration Curve" )   # Model's Actual Calibration Curve
    ...
    plt.ylabel( "Fraction of Positives | True Probability In Each Bin", fontsize = 10 )
